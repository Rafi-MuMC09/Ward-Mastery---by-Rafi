...
// ============================================================================
// SHARED CLINICAL POINT RENDERER
// Detects common text patterns already used across our content
// (clinicalExamNotes, gen_exam_causes, pediatrics) and styles them with
// visual hierarchy — zero data changes, pure presentation layer.
// ============================================================================
function cpBoldKeywords(text) {
    const keywords = [
        "NEVER", "ALWAYS", "MUST", "DO NOT MISS", "DO NOT(?! MISS)", "EMERGENCY",
        "normal", "abnormal", "Normal", "Abnormal",
        "RIGHT", "LEFT", "MEDIAL", "LATERAL", "ANTERIOR", "POSTERIOR", "PROXIMAL", "DISTAL"
    ];
    let result = text;
    keywords.forEach(kw => {
    ...
    // Bold measurements/numbers with units (e.g. "5th ICS", "50-95 bpm", "2-3 cm")
    result = result.replace(/\b(\d+(?:[-–]\d+)?\s?(?:cm|mm|bpm|sec|seconds|min|minutes|ICS|degrees|kg|%|z-score))\b/gi, (m, p1, offset, str) => {
        const before = str.slice(0, offset);
        if (before.lastIndexOf("<") > before.lastIndexOf(">")) return m;
        return "<strong style=\"color:#1d4ed8;\">" + m + "</strong>";
    });
    return result;
}

// ── MEDIA-AWARE WRAPPER ──────────────────────────────────────
function renderMediaItem(li, item) {
    const isObj = item && typeof item === "object" && "text" in item;
    const text = isObj ? item.text : item;
    const media = isObj ? item.media : null;

    renderClinicalPoint(li, text);

    if (media && media.length > 0) {
        const mediaWrap = document.createElement("div");
        mediaWrap.style.cssText = "margin:8px 0 4px;";
        media.forEach(function(m) {
            const fig = document.createElement("figure");
            fig.style.cssText = "margin:0 0 8px;";
            if (m.type === "video") {
                const video = document.createElement("video");
                video.src = m.url;
                video.controls = true;
                video.style.cssText = "width:100%;max-width:340px;border-radius:10px;border:2px solid #d1fae5;display:block;";
                fig.appendChild(video);
            } else {
                const img = document.createElement("img");
                img.src = m.url;
                img.loading = "lazy";
                img.style.cssText = "width:100%;max-width:340px;border-radius:10px;border:2px solid #d1fae5;display:block;cursor:pointer;";
            ...
            if (m.caption) {
                ...
                cap.style.cssText = "font-size:11px;color:#15803d;font-style:italic;margin-top:3px;";
                fig.appendChild(cap);
            }
...
// ── CONTENT CLASSIFIER ───────────────────────────────────────
// Looks at the FULL text (not just the prefix) to decide what kind of
// clinical point this is, so technique steps and warnings get styled
// correctly even when they don't carry an explicit "PROCEDURE:" /
// "EMERGENCY" label — e.g. "Place tips of fingers between..." or
// "NEVER palpate both carotids simultaneously — risk of cutting off
// cerebral blood flow" must be caught even mid-list, not just at a
// section's first line.
function cpClassifyPoint(text) {
    // Explicit section divider — checked first, takes priority over all else
    if (/^──\s*(.+?)\s*──/.test(text)) return "divider";

    // Explicit EMERGENCY / DO NOT MISS label
    if (/^EMERGENCY\b/i.test(text) || /DO NOT MISS/i.test(text)) return "emergency";

    // Danger / warning language ANYWHERE in the sentence — these are
    // genuine patient-safety warnings even without an "EMERGENCY" label.
    if (/\bNEVER\b/.test(text) ||
        /\brisk of\b/i.test(text) ||
        /\bobliterat/i.test(text) ||
        /\bcutting off\b/i.test(text) ||
        /\bsuspect (a |an )?[A-Z]/.test(text) ||
        /\bvagal reflex\b/i.test(text) ||
        /\bbradycardia\/syncope\b/i.test(text)) {
        return "warning";
    }

    // Explicit PROCEDURE: label
    if (/^PROCEDURE\s*:/i.test(text)) return "procedure";

    // Technique/instruction language — imperative verbs at the start,
    // OR strong technique markers anywhere in the sentence. This catches
    // hands-on steps that were never labeled "PROCEDURE:" but clearly are.
    if (/^(Place|Press|Count|Use|Listen|Palpate|Ask|Inspect|Measure|Apply|Roll|Flex|Cup|Stand|Feel|Look|Percuss|Auscultate|Examine|Position|Explain|Check)\b/i.test(text)) {
        return "procedure";
    }
    if (/\b(technique|best heard|best felt|with the (patient|pads|thumb|fingers)|using (the |pads|thumb|fingers))\b/i.test(text)) {
        return "procedure";
    }

    // IMCI-style severity classification — checked as two patterns since
    // "VERY SEVERE" must be matched as one unit before the generic
    // qualifier-prefix pattern (e.g. "COMPLICATED SEVERE...") is tried,
    // otherwise "VERY" gets mis-parsed as a qualifier on its own.
    const sevMatchVery = text.match(/^(VERY SEVERE)\b\s*([A-Z][A-Za-z\s\/]*?)?\s*[—–-]\s*(.*)$/);
    const sevMatchOther = text.match(/^([A-Z]+\s+)?(SEVERE|MODERATE|MILD|NO)\b\s*([A-Z][A-Za-z\s\/]*?)?\s*(?:\([A-Z]+\))?\s*[—–-]\s*(.*)$/);
    if (sevMatchVery || sevMatchOther) return "severity";

    // Arrow mapping (cause → finding / finding → disease)
    if (text.includes("→")) return "arrow";

    // Gender tag present inline
    if (/\[(Female|Male)\]/i.test(text)) return "gender";

    return "plain";
}

function renderClinicalPoint(li, text) {
    li.innerHTML = "";
    const kind = cpClassifyPoint(text);

    if (kind === "emergency") {
        li.style.cssText = "list-style:none;margin:6px 0;padding:8px 10px;background:#fef2f2;border:1px solid #fecaca;border-left:4px solid #dc2626;border-radius:6px;color:#7f1d1d;font-size:12px;line-height:1.6;font-weight:600;";
        li.innerHTML = "🚨 " + cpBoldKeywords(text);
        return;
    }

    if (kind === "warning") {
        li.style.cssText = "list-style:none;margin:6px 0;padding:8px 10px;background:#fff7ed;border:1px solid #fed7aa;border-left:4px solid #ea580c;border-radius:6px;color:#7c2d12;font-size:12px;line-height:1.6;";
        li.innerHTML = '<span style="font-weight:700;color:#c2410c;">⚠️</span> ' + cpBoldKeywords(text);
        return;
    }

    if (kind === "procedure") {
        const rest = text.replace(/^PROCEDURE\s*:/i, "").trim();
        li.style.cssText = "list-style:none;margin:6px 0;padding:8px 10px;background:#eff6ff;border:1px solid #bfdbfe;border-left:4px solid #2563eb;border-radius:6px;color:#1e3a8a;font-size:12px;line-height:1.6;";
        li.innerHTML = '<span style="font-weight:700;color:#1d4ed8;">🔧</span> ' + cpBoldKeywords(rest);
        return;
    }

    if (kind === "divider") {
        const dividerMatch = text.match(/^──\s*(.+?)\s*──(.*)$/);
        const label = dividerMatch[1];
        const remainder = dividerMatch[2].trim();
        li.style.cssText = "list-style:none;margin:12px 0 6px;padding:0;";
        li.innerHTML = '<div style="font-weight:800;font-size:11px;letter-spacing:0.5px;color:#92400e;background:#fef3c7;padding:5px 10px;border-radius:5px;display:inline-block;">' + label + '</div>'
            + (remainder ? '<div style="margin-top:4px;color:inherit;">' + cpBoldKeywords(remainder) + '</div>' : '');
        return;
    }
    ...
    if (kind === "severity") {
        const sevMatchVery = text.match(/^(VERY SEVERE)\b\s*([A-Z][A-Za-z\s\/]*?)?\s*[—–-]\s*(.*)$/);
        const sevMatchOther = text.match(/^([A-Z]+\s+)?(SEVERE|MODERATE|MILD|NO)\b\s*([A-Z][A-Za-z\s\/]*?)?\s*(?:\([A-Z]+\))?\s*[—–-]\s*(.*)$/);
        const qualifier = sevMatchVery ? "" : (sevMatchOther[1] || "").trim();
        const sevWord = sevMatchVery ? sevMatchVery[1] : sevMatchOther[2];
        const label = sevMatchVery ? (sevMatchVery[2] || "").trim() : (sevMatchOther[3] || "").trim();
        const rest = sevMatchVery ? sevMatchVery[3] : sevMatchOther[4];
        const colorMap = {
            "VERY SEVERE": "#dc2626", "SEVERE": "#dc2626",
            "MODERATE": "#d97706", "MILD": "#16a34a", "NO": "#16a34a"
        };
        const bgMap = { "#dc2626": "#fef2f2", "#d97706": "#fffbeb", "#16a34a": "#f0fdf4" };
        const color = colorMap[sevWord] || "#374151";
        const bg = bgMap[color] || "#f9fafb";
        const fullLabel = (qualifier ? qualifier + " " : "") + sevWord + (label ? " " + label : "");
        li.style.cssText = "list-style:none;margin:6px 0;padding:6px 10px;background:" + bg + ";border-left:4px solid " + color + ";border-radius:6px;font-size:12px;line-height:1.6;color:#1f2937;";
        li.innerHTML = '<span style="font-weight:800;color:' + color + ';">' + fullLabel + '</span> — ' + cpBoldKeywords(rest);
        return;
    }

    if (kind === "gender") {
        let withTags = text.replace(/\[(Female|Male)\]/gi, function(m, g) {
            const isF = g.toLowerCase() === "female";
            return '<span style="background:' + (isF ? "#fce7f3" : "#dbeafe") + ';color:' + (isF ? "#9d174d" : "#1e40af") + ';padding:1px 7px;border-radius:10px;font-size:10px;font-weight:700;">' + (isF ? "♀ Female" : "♂ Male") + '</span>';
        });
        li.style.cssText = "color:inherit;font-size:12px;line-height:1.6;";
        li.innerHTML = cpBoldKeywords(withTags);
        return;
    }
    ...
    if (kind === "arrow") {
        ...
        li.style.cssText = "color:inherit;font-size:12px;line-height:1.6;";
        li.innerHTML = '<strong style="color:#9a3412;">' + left + '</strong> → ' + cpBoldKeywords(right);
        return;
    }

    // Plain — still auto-bold key words/measurements, subtle background
    // so it's visually distinct from the colored category cards above it.
    li.style.cssText = "color:inherit;font-size:12px;line-height:1.7;padding:3px 0;";
    li.innerHTML = cpBoldKeywords(text);
}
