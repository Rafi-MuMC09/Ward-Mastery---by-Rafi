...
function cpBoldKeywords(text) {
    const keywords = ["NEVER", "ALWAYS", "MUST", "DO NOT MISS", "DO NOT(?! MISS)", "EMERGENCY", "normal", "abnormal", "Normal", "Abnormal"];
    let result = text;
    keywords.forEach(kw => {
...
// ── MEDIA-AWARE WRAPPER ──────────────────────────────────────
// Handles list items that may be either a plain string OR an object
// { text, media: [{ type, url, caption }] }. Renders the text using
// renderClinicalPoint (so all pattern-detection/styling still applies),
// then appends any attached images/videos directly below the text,
// inline within the same <li> — matching the in-context reference style.
...
function renderClinicalPoint(li, text) {
    ...
    if (/^EMERGENCY\b/i.test(text) || /DO NOT MISS/i.test(text)) {
    ...
    if (/^PROCEDURE\s*:/i.test(text)) {
        ...
        li.innerHTML = '<span style="font-weight:700;color:#1d4ed8;">🔧 PROCEDURE:</span> ' + cpBoldKeywords(rest);
        return;
    }
    ...
    const sevMatch = text.match(/^(SEVERE|VERY SEVERE|MODERATE|MILD|NO)\s+([A-Z][A-Z\s\/]+?)\s*[—–-]\s*(.*)$/);
    if (sevMatch) {
        const sevWord = sevMatch[1];
        const colorMap = {
        ...
        li.style.cssText = "list-style:none;margin:6px 0;padding:6px 10px;background:" + (bgMap[color] || "#f9fafb") + ";border-left:4px solid " + color + ";border-radius:6px;font-size:12px;line-height:1.6;color:#1f2937;";
        li.innerHTML = '<span style="font-weight:800;color:' + color + ';">' + sevMatch[1] + ' ' + sevMatch[2] + '</span> — ' + cpBoldKeywords(sevMatch[3]);
        return;
    }

    if (/\[(Female|Male)\]/i.test(text)) {
