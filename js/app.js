    ...
    if (data.clinicalExamNotes && data.clinicalExamNotes.length > 0) {
        data.clinicalExamNotes.forEach(section => {
            ...
            section.points.forEach(p => {
                const li = document.createElement("li");
                // Support both plain strings and {text, media} objects (same pattern as data.site)
                ...
                eUl.appendChild(li);
                // Render images/videos after their point, if provided
                if (p && p.media && p.media.length > 0) {
                    p.media.forEach(m => {
                        if (m.type === "image") {
                            ...
                            const img = document.createElement("img");
                            img.src = m.url;
                            img.alt = m.caption || pointText;
                            img.loading = "lazy";
                            img.style.cssText = "width:100%;max-width:340px;border-radius:10px;border:2px solid #bfdbfe;display:block;box-shadow:0 2px 8px rgba(0,0,0,0.10);";
                            img.onerror = function(){ this.style.display="none"; };
                            imgWrap.appendChild(img);
                            if (m.caption) {
                                const cap = document.createElement("div");
                                cap.textContent = m.caption;
                                cap.style.cssText = "font-size:11px;color:#1e40af;margin-top:4px;font-style:italic;";
                                imgWrap.appendChild(cap);
                            }
                            eUl.appendChild(imgWrap);
                        } else if (m.type === "video") {
                            const vidWrap = document.createElement("div");
                            vidWrap.style.cssText = "margin:8px 0 12px 0;";
                            const vid = document.createElement("video");
                            vid.src = m.url;
                            vid.controls = true;
                            vid.style.cssText = "width:100%;max-width:340px;border-radius:10px;border:2px solid #bfdbfe;display:block;";
                            vidWrap.appendChild(vid);
                            if (m.caption) {
                                const cap = document.createElement("div");
                                cap.textContent = m.caption;
